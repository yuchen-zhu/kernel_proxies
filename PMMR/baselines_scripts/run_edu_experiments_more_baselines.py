...
import numpy as np
from baselines.all_baselines import Poly2SLS, Vanilla2SLS, DirectNN, NNP2SLS, GMM, DeepIV, AGMM, NN2SLS, Bases2SLS, KRidge, Direct_discr
import os
import tensorflow
# from tabulate import tabulate
from MMR_proxy.util import ROOT_PATH, load_data, data_transform, data_inv_transform, split_into_bins
import random
# random.seed(527)
import argparse
...
def eval_ate_cat(model_cat, do_A_gt, bins, labels, EY_do_A_gt):
    print('bins: {}\n labels: {}\n do_A_gt: {}'.format(bins, labels, do_A_gt))
    do_A_gt_cat = split_into_bins(do_A_gt, bins=bins, labels=labels)
    print('do_A_gt_cat', do_A_gt_cat)
    EYhat_do_A = model_cat.predict(do_A_gt_cat.reshape(-1,1))
...
def run_experiment(scenario_name,mid,repid,trainsz, data_seed, num_reps=10, seed=527, training=False, args=None, PATH=None):
    ...
    # data for discrete E[Y|A]
    Y_cat, X_cat = train.y[:trainsz].reshape(-1,1), split_into_bins(train.a[:trainsz], bins=np.array([-0.5,0.5,1.5,2.5]), labels=np.array([0,1,2])).reshape(-1,1)
    # print('X_cat: ', X_cat)
    Z_cat, W_cat = train.z[:trainsz].reshape(trainsz,-1), None
    ...
    for rep in range(num_reps):
        ...
        if training:
            ...
            # else:
            #     pass
            for methods_list, tag in [(methods_cat, 'cat'), (methods_direct, 'direct'), (methods_2s, '2sls'), (methods_covar, 'covar'), (methods_covar_w, 'covar_w')]:
                ...
                for method_name, method in methods_list:
                    ...
                    if tag == 'direct' or tag == 'covar' or tag =='covar_w':
                        ...
                        print('method: {}, tag: {}'.format(method_name, tag))
                        causal_mae, EYhat_do_A, causal_std, causal_rel_err = eval_ate_cat(model_cat=model, do_A_gt=do_A_unscaled,
                                                                                          bins=np.append(do_A_cat, do_A_cat[-1].squeeze()+1)-0.5,
                                                                                          labels=np.array([0,1,2]),
                                                                                          EY_do_A_gt=EY_do_A_gt)
                    if rep == 0:
