...
from MMR_proxy.util import ROOT_PATH, load_data, bundle_az_aw, get_median_inter_mnist, _sqdist, nystrom_decomp_from_orig, chol_inv,\
    visualise_ATEs, scale_all, indicator_kern
...
def make_gaussian_prodkern(arr1, arr2, sigma):
    ...
    K_bin_list = []
    for dim in range(bin_idx, dims):
        # print('az_bin_idx: ', az_bin_idx)
        # print('az at dim {}'.format(dim), Z[:,dim])
        K_ = indicator_kern(arr1[:,dim], arr2[:,dim])
        K_bin_list.append(K_)
        print('sum W_dim: ', np.sum(K_))
    K_bin = 1 if len(K_bin_list) == 0 else np.prod(np.array(K_bin_list), axis=0)
    K = K_bin * K

    return K


def process_data(train_size, dev_size, test_size, args, data_seed, **kwargs):
    t1 = time.time()
    ...
    if args.sem == 'ab':
        z_bin_idx, w_bin_idx = kwargs['z_bin_idx'], kwargs['w_bin_idx']
        A_scaled, Y_scaled, Z_cts_scaled, W_cts_scaled, test_A_scaled, test_Y_scaled, test_Z_cts_scaled, test_W_cts_scaled, A_scaler, Y_scaler, Z_scaler, W_scaler = \
            scale_all(train_A=train.a[:train_size], train_Y=train.y[:train_size],
                      train_Z=train.z[:train_size, :z_bin_idx], train_W=train.w[:train_size, :w_bin_idx],
                      test_A=test.a[:test_size], test_Y=test.y[:test_size], test_Z=test.z[:test_size, :z_bin_idx],
                      test_W=test.w[:test_size, :w_bin_idx])
        Z_scaled, test_Z_scaled = np.concatenate([Z_cts_scaled.reshape(Z_cts_scaled.shape[0], -1),
                                                  train.z[:train_size, z_bin_idx:]], axis=-1), \
                                  np.concatenate([test_Z_cts_scaled.reshape(test_Z_cts_scaled.shape[0], -1),
                                                  test.z[:test_size, z_bin_idx:]], axis=-1)
        W_scaled, test_W_scaled = np.concatenate([W_cts_scaled, train.w[:train_size, w_bin_idx:]], axis=-1), \
                                  np.concatenate([test_W_cts_scaled, test.w[:test_size, w_bin_idx:]], axis=-1)
        train_Y, test_Y = Y_scaled.reshape(-1,1), test_Y_scaled.reshape(-1,1)
        ...
        W_marginal = W_scaled[:args.w_marginal_size].reshape(args.w_marginal_size, -1)
        ...
        train_Y, test_Y = train.y[:train_size].reshape(-1, 1), test.y.reshape(-1,1)[:test_size]  # Y always has only 1 feature
...
def experiment(seed, data_seed, param_l_arg, train_size, dev_size, test_size, nystr, args, PATH):
    ...
    al_default = compute_bandwidth_median_dist(X=X[:, :aw_bin_idx])

    EYEN = np.eye(X.shape[0])

    print('Z shape: ', Z.shape)
    # ak = compute_bandwidth_median_dist(Z) / [1.5, 1.5, 1.5]
    ak = compute_bandwidth_median_dist(Z[:, :az_bin_idx]) / W_kern_divisor
    N2 = X.shape[0] ** 2
    print('making W_.')
