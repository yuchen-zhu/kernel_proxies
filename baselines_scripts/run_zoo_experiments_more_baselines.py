...
from PMMR.util import ROOT_PATH, load_data, data_transform, calculate_off_set, visualise_ATEs
# random.seed(527)
import argparse
...
def eval_model(model, test, tag):
    if tag == "direct":
        ...
        y_pred_test = model.predict(np.concatenate([test.a.reshape(test.a.shape[0],-1), test.w.reshape(test.w.shape[0], -1)], axis=-1))
        mse = float(((y_pred_test.squeeze() - test.y) ** 2).mean())
    elif tag == 'covar':
        y_pred_test = model.predict(test.a.reshape(test.a.shape[0],-1), context=np.concatenate([test.z.reshape(test.z.shape[0], -1), test.w.reshape(test.w.shape[0], -1)], axis=-1))
        mse = float(((y_pred_test.squeeze() - test.y) ** 2).mean())
    elif tag == 'covar_w':
        y_pred_test = model.predict(test.a.reshape(test.a.shape[0],-1), context=test.w.reshape(test.w.shape[0],-1))
        mse = float(((y_pred_test.squeeze() - test.y) ** 2).mean())
    else:
...
def eval_ate_direct(model_direct, do_A, context, EY_do_A, Y_scaler):
    ...
    offset = calculate_off_set(EY_do_A, EYhat_do_A)
    res_change = EYhat_do_A + offset

    offset_mae = np.mean(np.abs(EY_do_A.squeeze() - res_change.squeeze()))

    return mae, offset_mae, EYhat_do_A, std, mean_rel_err
...
def eval_ate_2sls(model_2sls, do_A, w, EY_do_A_gt, Y_scaler):
    ...
    offset = calculate_off_set(EY_do_A_gt, EYhat_do_A)
    res_change = EYhat_do_A + offset

    offset_mae = np.mean(np.abs(EY_do_A_gt.squeeze() - res_change.squeeze()))
...
def plotting(EYhat_do_A, do_A, EY_do_A, PATH, method_name, train_sz, args, seed, data_seed):
    ...
    #     print('making {}'.format(os.path.join(PATH, args.sem+'_seed{}'.format(data_seed))))
    #     raise ValueError
    plt.figure()
...
def run_experiment(scenario_name,mid,repid,trainsz, data_seed, num_reps=10, seed=527, training=False, args=None, PATH=None):
    ...
    scale_all(train_A=train.a[:trainsz].reshape(trainsz, -1), train_Y=train.y[:trainsz].reshape(trainsz, -1), train_Z=train.z[:trainsz].reshape(trainsz, -1), train_W=train.w[:trainsz].reshape(trainsz, -1),
    ...
    # methods_direct += [("DirectNN", DirectNN())]
    # methods_covar += [("AdjustNN", DirectNN())]
    # methods_covar_w += [("AdjustNN_W", DirectNN())]
            ...
            for methods_list, tag in [(methods_direct, 'direct'), (methods_2s, '2sls'), (methods_covar, 'covar'), (methods_covar_w, 'covar_w')]:
                ...
                for method_name, method in methods_list:
                    if method_name not in results_dict:
                        results_dict[method_name] = {'causal_mae':{}, 'causal_std': {}, 'causal_rel_err': {}, 'causal_est': {}, 'offset_mae': {}}
                    ...
                    if tag == 'direct' or tag == 'covar' or tag =='covar_w':
                        ...
                        causal_mae, offset_mae, EYhat_do_A, causal_std, causal_rel_err = eval_ate_direct(model_direct=model, do_A=do_A, context=context, EY_do_A=EY_do_A_gt, Y_scaler=Y_scaler)
                    elif tag == '2sls':
                        print('method: {}, tag: {}'.format(method_name, tag))
                        causal_mae, offset_mae, EYhat_do_A, causal_std, causal_rel_err = eval_ate_2sls(model_2sls=model, do_A=do_A, w=W_scaled[:50].reshape(50, -1), EY_do_A_gt=EY_do_A_gt, Y_scaler=Y_scaler)
                    if rep == 0:
                    ...
                    results_dict[method_name]['offset_mae'][rep] = offset_mae

                    test_mse = eval_model(model, test, tag=tag)
                    # print('BASELINE COLLECT: ', EYhat_do_A_collect)
                    model_type_name = type(model).__name__
                    print("Test MSE of %s: %f, Causal MAE: %f, Offset MAE: %f" % (method_name, test_mse, causal_mae, offset_mae))
                    ...
                    performance_log_file.write(
                        "Method name: {}, rep: {}, test mse: {}, offset mae: {}, causal mae: {}, causal std: {}, causal rel err: {}\n".format(method_name, rep, test_mse, offset_mae, causal_mae, causal_std, causal_rel_err))
...
if __name__ == "__main__":
    ...
    sname, train_sizes = 'sim_1d_no_x', [200, 500, 1000]
