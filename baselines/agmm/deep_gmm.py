    ...
    def fit(self, data_z, data_p, data_y):
        ...
        with tf.compat.v1.Session() as sess:
            ...
            for step in range(1, self._num_steps + 1):
                ...
                if step % self._check_loss_step == 0 or step == 1 or step == self._num_steps:
                    ...
                    if new_loss <= loss:
                        if num_treatments == 1:
                            ...
                            saver.save(sess, "./kpv_rbfNonLinearYFullTrain10000scaledSig2NewTest/model_best.ckpt")
                            loss = new_loss
                ...
                if step in avg_store_steps: #step > .2 * self._num_steps:
                    if num_treatments == 1:
                        ...
                        saver.save(sess, "./kpv_rbfNonLinearYFullTrain10000scaledSig2NewTest/model_{}.ckpt".format(step))
            ...
            if num_treatments == 1:
                ...
                saver.save(sess, "./kpv_rbfNonLinearYFullTrain10000scaledSig2NewTest/model_final.ckpt")
    ...
    def predict(self, data_p, model='avg'):
        ...
        with tf.compat.v1.Session() as sess:
            ...
            if model == 'avg':
                output = []
                for it, ckp in enumerate(self._checkpoints):
                    saver.restore(sess, "./kpv_rbfNonLinearYFullTrain10000scaledSig2NewTest/model_{}.ckpt".format(ckp))
                    output.append(sess.run(self.gmm_graph.modeler.output, feed_dict={self.P: data_p}))
                return np.mean(np.array(output), axis=0).flatten()
            elif model == 'best':
                saver.restore(sess, "./kpv_rbfNonLinearYFullTrain10000scaledSig2NewTest/model_best.ckpt")
                return sess.run(self.gmm_graph.modeler.output, feed_dict={self.P: data_p}).flatten()
            else:
                saver.restore(sess, "./kpv_rbfNonLinearYFullTrain10000scaledSig2NewTest/model_final.ckpt")
                return sess.run(self.gmm_graph.modeler.output, feed_dict={self.P: data_p}).flatten()
