...
class AbstractBaseline(object):
    ...
    def predict(self, x, context=None):
        ...
        if self._model is None:
            ...
            if result.ndim != 2 and result.ndim != 1:
...
class AbstractLVMBaseline(object):
    def __init__(self):
        self._model = None
        self._a_dim = None

    def fit(self, a, y, z, w):
        self._check_arguments(a, y, z, w)
        self._a_dim = a.shape[1]
        time = self._fit(a, y, z, w)
        return self, time
    ...
    def predict(self, a):
        ...
        if self._model is None:
            raise AttributeError("Model has not been fit!")
        elif self._a_dim != a.shape[1]:
            raise ValueError(
                "Was fitted with %d-dimensional a, but now got called with %-d-dimensional a" % (
                self._a_dim, a.shape[1]))
        else:
            result = self._predict(a)
            if result.ndim != 2 and result.ndim != 1:
                raise ValueError("Class returned incorrect shape in predict()!")
            return result
    ...
    def _check_arguments(a, y, z, w):
        ...
        if not all([var.ndim == 2 for var in all_var]):
            raise ValueError(
                "All variables need to be 2-dimensional, but dimensions given are " + str(
                    [var.ndim for var in all_var]))
        if not all([var.shape[0] == a.shape[0] for var in all_var]):
            raise ValueError(
                "All variables need to have same number of samples, but sizes given are " + str(
                    [var.shape for var in all_var]))
        if not y.shape[1] == 1:
