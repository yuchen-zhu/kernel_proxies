...
bn_max, bn_min, b_mesh_size = (40, 33, 3)
...
def experiment(sname, seed, param_arg, train_size, nystr=False, args=None, PATH=None, hparam=None):
    ...
    def get_causal_effect(params, do_A, w, Y_scaler):
        ...
        ate_L0_cts = _sqdist(aw_rep[:, :-1], X[:, :-1])
        ate_L_cts = bl * bl * np.exp(-ate_L0_cts/ al/ al/ 2)
        ate_L_bin = indicator_kern(aw_rep[:, -1], X[:, -1])
    ...
    def mmr_loss(al, bl, alpha, y_test, aw_test, az_test):
        ...
        ak = get_median_inter_mnist(az_test[:, :1])
        N2 = y_test.shape[0] ** 2
        N = y_test.shape[0]
        W0 = _sqdist(az_test[:, :1], None)
        print('av kernel indicator: ', args.av_kernel)
        K_cts = np.exp(-W0 / ak / ak / 2)
        del W0

        K_list = []
        for dim in range(1, az_test.shape[-1]):
    ...
    A_scaled, Y_scaled, _, W_cts_scaled, test_A_scaled, test_Y_scaled, _, test_W_cts_scaled, A_scaler, Y_scaler, Z_scaler, W_scaler = \
    scale_all(train_A=train.a[:train_size], train_Y=train.y[:train_size], train_Z=train.z[:train_size], train_W=train.w[:train_size, :-1],
              test_A=test.a[:test_size], test_Y=test.y[:test_size], test_Z=test.z[:test_size], test_W=test.w[:test_size, :-1])

    Z_scaled, test_Z_scaled = train.z[:train_size], test.z[:test_size]
    W_scaled, test_W_scaled = np.concatenate([W_cts_scaled, train.w[:train_size, -1:]], axis=-1), np.concatenate([test_W_cts_scaled, test.w[:test_size, -1:]], axis=-1)
    ...
    L0_cts, test_L0_cts = _sqdist(X[:, :-1], None), _sqdist(test_X[:, :-1], X[:, :-1])
    L_bin, test_L_bin = indicator_kern(X[:, -1], X[:, -1]), indicator_kern(test_X[:, -1], X[:, -1])
    t4 = time.time()
    print('t4 - t3', t4-t3)
    # measure time
