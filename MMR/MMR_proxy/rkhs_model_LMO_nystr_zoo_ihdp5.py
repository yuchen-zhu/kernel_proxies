...
bn_max, bn_min, b_mesh_size = (5000, 30, 20)
...
def hparam_selection_from_metric_votes(mmr_u, mmr_u_supp, lmo, hparam_arr):
    ranks_mmr_u = rank_nums_in_array(mmr_u.squeeze())
    # ranks_mmr_u_supp = rank_nums_in_array(mmr_u_supp.squeeze())
    # ranks_lmo = rank_nums_in_array(lmo.squeeze())
    # sum_ranks = ranks_mmr_u + ranks_mmr_u_supp + ranks_lmo
    sum_ranks = ranks_mmr_u
...
def get_best_hparam(results_dict):
    ...
    mmr_us = np.array(results_dict['mmr_u'])[:, -1]
    mmr_u_supps = None
    lmos = np.array(results_dict['lmo'])[:, -1]
    return hparam_selection_from_metric_votes(mmr_u=mmr_us, mmr_u_supp=mmr_u_supps, lmo=lmos, hparam_arr=hparam_arr)
...
def hyparameter_selection(a_diffmin, a_diffmax, a_mesh_size, b_min, b_max, b_mesh_size, args, PATH, train_size):
    if args.hparam == 'cube':
        ...
        causal_effect_mean_abs_err, err_in_expectation, al_median_dist, causal_effect_est, mse_standard, \
...
def get_results(EYhat_do_A, EY_do_A_gt, train_sz, mse_standard, mmr_v, mmr_u, lmo, params, args, PATH):
    ...
    summary_file.write("al: {}, bl: {}, causal_mae_: {}, causal_std: {}, causal_rel_err: {}\n"
                       ...
                       "causal_gt: {}\n".format(params[0], params[1], causal_effect_mean_abs_err, causal_std,
                                                 causal_rel_err, mmr_v, mmr_u, mse_standard, lmo, EYhat_do_A, EY_do_A_gt))
    summary_file.close()
...
def experiment(sname, seed, param_arg, train_size, nystr=False, args=None, PATH=None, hparam=None):
    ...
    def LMO_err(params, M=10):
        ...
        L_cts = bl * bl * np.exp(-L0_cts / al / al / 2) + 1e-6 * EYEN
        L = L_cts * L_bin
        if nystr:
            tmp_mat = L @ eig_vec_K
    ...
    def get_causal_effect(params, do_A, w, Y_scaler):
        ...
        ate_L0_cts = _sqdist(aw_rep[:, :aw_bin_idx], X[:, :aw_bin_idx])
        ate_L_cts = bl * bl * np.exp(-ate_L0_cts/ al/ al/ 2)

        ate_L_bin_list = []
        for dim in range(aw_bin_idx, X.shape[-1]):
            ate_L_bin = indicator_kern(aw_rep[:, dim], X[:, dim])
            ate_L_bin_list.append(ate_L_bin)
        ate_L_bin = np.prod(ate_L_bin_list, axis=0)
        ate_L = ate_L_bin * ate_L_cts
        h_out = ate_L @ alpha
    ...
    def compute_losses(params, AW_test, AZ_test, Y_test):
        ...
        # standard mse
        mse_L0 = _sqdist(AW_test[:, :-1], X[:, :-1])
        mse_L_bin = indicator_kern(AW_test[:, -1], X[:, -1])
        mse_L_cts = bl * bl * np.exp(-mse_L0 / al / al / 2)
        mse_L = mse_L_cts * mse_L_bin
        mse_h = mse_L @ alpha
        mse_standard = np.mean((test_Y.flatten() - mse_h.flatten()) ** 2)

        mmr_v, mmr_u = mmr_loss(al=al, bl=bl, alpha=alpha, y_test=Y_test, aw_test=AW_test, az_test=AZ_test)
    ...
    def mmr_loss(al, bl, alpha, y_test, aw_test, az_test):
        ...
        ak = get_median_inter_mnist(az_test[:, :az_bin_idx])
        N2 = y_test.shape[0] ** 2
        N = y_test.shape[0]
        W0 = _sqdist(az_test[:, :az_bin_idx], None)
        print('av kernel indicator: ', args.av_kernel)
        K_cts = np.exp(-W0 / ak / ak / 2)
        del W0

        K_list = []
        for dim in range(az_bin_idx, az_test.shape[-1]):
            K_list.append(indicator_kern(az_test[:, dim], az_test[:, dim]))
        K = np.prod(np.array(K_list), axis=0) * K_cts
    ...
    scale_all(train_A=train.a[:train_size], train_Y=train.y[:train_size], train_Z=train.z[:train_size, :z_bin_idx], train_W=train.w[:train_size, :w_bin_idx],
              test_A=test.a[:test_size], test_Y=test.y[:test_size], test_Z=test.z[:test_size, :z_bin_idx], test_W=test.w[:test_size, :w_bin_idx])

    Z_scaled, test_Z_scaled = np.concatenate([Z_cts_scaled.reshape(Z_cts_scaled.shape[0], -1), train.z[:train_size, z_bin_idx:]], axis=-1), \
                              np.concatenate([test_Z_cts_scaled.reshape(test_Z_cts_scaled.shape[0], -1), test.z[:test_size, z_bin_idx:]], axis=-1)
    W_scaled, test_W_scaled = np.concatenate([W_cts_scaled, train.w[:train_size, w_bin_idx:]], axis=-1), np.concatenate([test_W_cts_scaled, test.w[:test_size, w_bin_idx:]], axis=-1)
    ...
    ak = get_median_inter_mnist(Z[:, :az_bin_idx])
    N2 = X.shape[0] ** 2
    W0 = _sqdist(Z[:, :az_bin_idx], None)
    print('av kernel indicator: ', args.av_kernel)
    # W = np.exp(-W0 / ak0 / ak0 / 2) / N2 if not args.av_kernel \
    #     else (np.exp(-W0 / ak0 / ak0 / 2) + np.exp(-W0 / ak0 / ak0 / 200) + np.exp(-W0 / ak0 / ak0 * 50)) / 3 / N2
    W_cts = np.exp(-W0 / ak / ak / 2) / N2 if not args.av_kernel \
        else (np.exp(-W0 / ak / ak / 2) + np.exp(-W0 / ak / ak / 200) + np.exp(-W0 / ak / ak * 50)) / 3 / N2
    #
    del W0
    W_bin_list = []
    for dim in range(az_bin_idx, Z.shape[-1]):
        print('az_bin_idx: ', az_bin_idx)
        print('az at dim {}'.format(dim), Z[:,dim])
        W_ = indicator_kern(Z[:,dim], Z[:,dim])
        W_bin_list.append(W_)
        print('sum W_dim: ', np.sum(W_))
    W = np.prod(np.array(W_bin_list), axis=0) * W_cts

    print('sum W: ', np.sum(W))

    L0_cts, test_L0_cts = _sqdist(X[:, :aw_bin_idx], None), _sqdist(test_X[:, :aw_bin_idx], X[:, :aw_bin_idx])

    L_bin_list, test_L_bin_list = [], []
    for dim in range(aw_bin_idx, X.shape[-1]):
        L_bin, test_L_bin = indicator_kern(X[:, dim], X[:, dim]), indicator_kern(test_X[:, dim], X[:, dim])
        L_bin_list.append(L_bin)
        test_L_bin_list.append(test_L_bin)
    L_bin = np.prod(np.array(L_bin_list), axis=0)
    test_L_bin = np.prod(np.array(test_L_bin_list), axis=0)
    t4 = time.time()
    print('t4 - t3', t4-t3)
    # measure time
    ...
    #     os.makedirs(os.path.join(PATH, str(date.today()), args.sem), exist_ok=True)
        ...
        al, bl = get_median_inter_mnist(X[:, :-1]) + param_arg[0], param_arg[1]
        print('bandwidth = ', al)
        # al = 8.4787
...
if __name__ == '__main__':
    ...
    for train_size in [1500]:
        ...
        for data_seed in [600, 700, 800, 900, 1000]:
            for sname in snames:
                PATH = ROOT_PATH + "/MMR_proxy/results/zoo/" + sname + "/"
                if not os.path.exists(os.path.join(PATH, str(date.today()), args.sem+'_seed'+str(data_seed))):
                    os.makedirs(os.path.join(PATH, str(date.today()), args.sem+'_seed'+str(data_seed)), exist_ok=True)
