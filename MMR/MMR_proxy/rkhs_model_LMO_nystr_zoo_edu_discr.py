...
bn_max, bn_min, b_mesh_size = (150, 2, 3)
...
def hyparameter_selection(a_diffmin, a_diffmax, a_mesh_size, b_min, b_max, b_mesh_size, args, PATH, train_size):
    if args.hparam == 'cube':
        ...
        causal_effect_mean_abs_err, al_median_dist, causal_effect_est, mse_standard, mmr_v, mmr_u, lmo = experiment(sname=sname, seed=seed, param_arg=[],
                                                                         train_size=train_size,
                                                                         nystr=(False if train_size < 500 else True),
                                                                         args=args, PATH=PATH, hparam=args.hparam)
        # if err_in_expectation_best is None or err_in_expectation < err_in_expectation_best:
...
def experiment(sname, seed, param_arg, train_size, nystr=False, args=None, PATH=None, hparam=None):
    ...
    def callback0(params, timer=None):
        ...
        if Nfeval % 1 == 0:
            ...
            test_L = bl * bl * np.exp(-test_L0 / al / al / 2) * test_L_bin
    ...
    def get_causal_effect(params, do_A, w, Y_scaler):
        ...
        al, bl = params
        print('al, bl = ', params)
        L_cts = bl * bl * np.exp(-L0 / al / al / 2) + 1e-6 * EYEN
        L = L_cts * L_bin
        if nystr:
            alpha = EYEN - eig_vec_K @ np.linalg.inv(
                eig_vec_K.T @ L @ eig_vec_K / N2 + np.diag(1 / eig_val_K / N2)) @ eig_vec_K.T @ L / N2
        ...
        ate_L0 = _sqdist(aw_rep[:, 1:], X[:, 1:])
        ate_L_bin = indicator_kern(aw_rep[:, 0], X[:, 0])
        ate_L = bl * bl * np.exp(-ate_L0/ al/ al/ 2) * ate_L_bin
        h_out = ate_L @ alpha
    ...
    def compute_losses(params, AW_test, AZ_test, Y_test):
        ...
        L = (bl * bl * np.exp(-L0 / al / al / 2) + 1e-6 * EYEN) * L_bin
        if nystr:
            alpha = EYEN - eig_vec_K @ np.linalg.inv(
                eig_vec_K.T @ L @ eig_vec_K / N2 + np.diag(1 / eig_val_K / N2)) @ eig_vec_K.T @ L / N2
            alpha = alpha @ W_nystr @ Y * N2
        ...
        mse_L_bin = indicator_kern(AW_test[:, 0], X[:, 0])
        mse_L = bl * bl * np.exp(-mse_L0 / al / al / 2) * mse_L_bin
        mse_h = mse_L @ alpha
    ...
    def mmr_loss(ak, al, bl, alpha, y_test, aw_test, az_test):
        ...
        W_bin = indicator_kern(az_test[:, 0], az_test[:, 0])
    ...
    _, Y_scaled, Z_scaled, W_scaled, _, test_Y_scaled, test_Z_scaled, test_W_scaled, _, Y_scaler, Z_scaler, W_scaler = \
    ...
    # test_Y = test.y
    AZ_train, AW_train = bundle_az_aw(train.a[:train_size].reshape(-1,1), Z_scaled, W_scaled)
    AZ_test, AW_test = bundle_az_aw(test.a[:test_size].reshape(-1,1), test_Z_scaled, test_W_scaled)
    ...
    W_bin = indicator_kern(Z[:, 0], Z[:, 0])

    W = W_cts * W_bin

    del W0
    L0, test_L0 = _sqdist(X[:, 1:], None), _sqdist(test_X[:, 1:], X[:, 1:])
    L_bin, test_L_bin = indicator_kern(X[:, 0], X[:, 0]), indicator_kern(test_X[:, 0], X[:, 0])
    t4 = time.time()
    print('t4 - t3', t4-t3)
    # measure time
