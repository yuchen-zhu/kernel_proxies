...
# b_max, b_min = (10, 1)
a_diffmax, a_diffmin, a_mesh_size = (0., -1., 2)
bn_max, bn_min, b_mesh_size = (200, 2, 30)
...
def hparam_selection_from_metric_votes(mmr_v, mmr_v_supp, lmo, hparam_arr, mse_standard):
    ranks_mmr_v = rank_nums_in_array(mmr_v.squeeze())
    # ranks_mse_standard = rank_nums_in_array(mse_standard.squeeze())
...
def get_best_hparam(results_dict):
    ...
    mse_standard = np.array(results_dict['mse_standard'])[:, -1]
    return hparam_selection_from_metric_votes(mmr_v=mmr_vs, mmr_v_supp=mmr_v_supps, lmo=lmos, hparam_arr=hparam_arr, mse_standard=mse_standard)
