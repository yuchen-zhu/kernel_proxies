import os
import numpy as np
from MMR_proxy.util import ROOT_PATH, load_data, bundle_az_aw, get_median_inter_mnist, _sqdist, nystrom_decomp_from_sub, chol_inv,\
    visualise_ATEs
...
w_marginal_size = 50
nystr_M = 1
log_al_bounds, log_bl_bounds = np.array([-10., 0.]), np.array([-5, 6.5])
nystr_thresh = 500
seed = 527

al_diff_search_range = [0., 0., 1]
bl_search_range = [0.3, 15., 300]
train_sizes = [5000]
# data_seeds = np.arange(100, 101, 100)
...
parser.add_argument('--JITTER', type=float, default=JITTER, help='set jitter value to stabilise kernel inversion.')
...
def process_data(train_size, dev_size, test_size, args, data_seed):
    t1 = time.time()

    # loads all data
    train, dev, test = load_data(os.path.join(ROOT_PATH, "data/zoo", args.scenario_name, 'main_{}_seed{}.npz'.format(args.sem, data_seed)))
    ...
    do_A = np.load(ROOT_PATH + "/data/zoo/" + args.scenario_name + "/do_A_{}_seed{}.npz".format(args.sem, data_seed))[
        'do_A'].reshape(-1,1)
    EY_do_A_gt = \
    np.load(ROOT_PATH + "/data/zoo/" + args.scenario_name + "/do_A_{}_seed{}.npz".format(args.sem, data_seed))[
        'gt_EY_do_A'].reshape(-1,1)
...
def load_err_in_expectation_metric(args, data_seed):
    t1 = time.time()
    axzy = np.load(ROOT_PATH + "/data/zoo/" + args.scenario_name + '/cond_exp_metric_{}_seed{}.npz'.format(args.sem, data_seed))['axzy']
    w_samples = np.load(ROOT_PATH + "/data/zoo/" + args.scenario_name + '/cond_exp_metric_{}_seed{}.npz'.format(args.sem, data_seed))['w_samples']
    y_samples = np.load(ROOT_PATH + "/data/zoo/" + args.scenario_name + '/cond_exp_metric_{}_seed{}.npz'.format(args.sem, data_seed))['y_samples']
    y_axz = axzy[:, -1]
    ax = axzy[:, :2]
    t2 = time.time()
...
def load_test_supp_metric(args, data_seed):
    """
...
def compute_alpha(train_size, eig_vec_K, X, Y, W, eig_val_K, nystr, params_l):
    N2 = train_size ** 2
    ...
    if nystr:
        random_indices = np.sort(np.random.choice(range(train_size), args.nystr_M, replace=False))
        assert X[random_indices].shape[0] == args.nystr_M
        assert X[random_indices].shape[1] == X.shape[1]
        assert X.ndim == X[random_indices].ndim
        L_nm = make_gaussian_prodkern(X, X[random_indices], sigma=al)
        assert L_nm.shape[0] == train_size
        L_mm = make_gaussian_prodkern(X[random_indices], X[random_indices], sigma=al)
        eig_val_L, eig_vec_L = nystrom_decomp_from_sub(G_mm=L_mm, G_nm=L_nm, N=train_size)
        inv_eig_val_K = np.diag(1 / eig_val_K / N2)
        # L_nystr = bl * bl * eig_vec_L @ np.diag(eig_val_L) @ eig_vec_L.T + JITTER * EYEN
        alpha = EYEN - eig_vec_K @ np.linalg.inv(
            eig_vec_K.T @ (bl * bl * eig_vec_L @ np.diag(eig_val_L) @ eig_vec_L.T + JITTER * EYEN) @ eig_vec_K / N2 + np.diag(1 / eig_val_K / N2)) @ eig_vec_K.T @ (bl * bl * eig_vec_L @ np.diag(eig_val_L) @ eig_vec_L.T + JITTER * EYEN) / N2
        alpha = alpha @ (eig_vec_K @ np.diag(eig_val_K) @ eig_vec_K.T / N2) @ Y * N2
    else:
        K_L = make_gaussian_prodkern(X, X, al)
        L = bl * bl * K_L + JITTER * EYEN  # L = (1/lambda * 1/(n^2)) * L_true
...
def LMO_err_global(log_params_l, train_size, W, eig_vec_K, eig_val_K, inv_eig_val_K, X, Y, Z, nystr, offset, ak, M=10):
    EYEN = np.eye(train_size)
    N2 = train_size ** 2
    ...
    if nystr:
        ...
        inv_eig_val_K = np.diag(1 / eig_val_K / N2)
        # L_nystr = bl * bl * eig_vec_L @ np.diag(eig_val_L) @ eig_vec_L.T + JITTER * EYEN
        tmp_mat = (bl * bl * eig_vec_L @ np.diag(eig_val_L) @ eig_vec_L.T + JITTER * EYEN) @ eig_vec_K
        C = (bl * bl * eig_vec_L @ np.diag(eig_val_L) @ eig_vec_L.T + JITTER * EYEN) - tmp_mat @ anp.linalg.inv(eig_vec_K.T @ tmp_mat / N2 + inv_eig_val_K) @ tmp_mat.T / N2
        c = C @ (eig_vec_K @ np.diag(eig_val_K) @ eig_vec_K.T / N2) * N2
        del tmp_mat
    else:
        LWL_inv = chol_inv(L @ W @ L + L / N2 + JITTER * EYEN)
        C = L @ LWL_inv @ L / N2
    ...
    lmo_err = 0
    N = 0
    W_tmp = make_gaussian_prodkern(Z, Z, sigma=ak) / N2
    for ii in range(1):
        idxs = np.arange(X.shape[0])
        for i in range(0, X.shape[0], M):
            indices = idxs[i:i + M]
            K_i = W_tmp[np.ix_(indices, indices)] * N2
...
def compute_losses(params_l, ax, w_samples, y_samples, y_axz, x_on,
                   ...
                   X, Y, Z, W, eig_vec_K, eig_val_K, inv_eig_val_K, nystr, test_Y, ak, alpha, offset, w_dim, args):
    ...
    lmo_err = LMO_err_global(log_params_l=log_params, train_size=X.shape[0], W=W,
                             eig_vec_K=eig_vec_K, eig_val_K=eig_val_K, inv_eig_val_K=inv_eig_val_K, X=X, Y=Y, Z=Z, nystr=nystr, M=10, offset=offset, ak=ak)
...
def get_results(do_A, EYhat_do_A, EY_do_A_gt, train_sz, err_in_expectation, mse_alternative, mse_standard, mse_supp,
                mmr_v_supp, mmr_v, mmr_u_supp, mmr_u, lmo, params_l, args, PATH, data_seed):
    plt.figure()
    ...
    summary_file = open(
        os.path.join(PATH, str(date.today()), args.sem+'_seed'+str(data_seed), "summary_trainsz{}_nystrom_hparam{}_offset{}.txt".format(train_sz, args.hparam, args.offset)),
        "a")
...
def experiment(seed, data_seed, param_l_arg, train_size, dev_size, test_size, nystr, args, PATH):
    np.random.seed(seed)
    random.seed(seed)
    ...
    ak = compute_bandwidth_median_dist(Z)
    N2 = X.shape[0] ** 2
    ...
    if nystr:
        random_indices = np.sort(np.random.choice(range(train_size), args.nystr_M, replace=False))
        assert Z[random_indices].shape[0] == args.nystr_M
        assert Z[random_indices].shape[1] == Z.shape[1]
        assert Z.ndim == Z[random_indices].ndim
        W_nm = make_gaussian_prodkern(Z, Z[random_indices], sigma=ak)
        assert W_nm.shape[0] == train_size
        W_mm = make_gaussian_prodkern(Z[random_indices], Z[random_indices], sigma=ak)
        eig_val_K, eig_vec_K = nystrom_decomp_from_sub(G_mm=W_mm, G_nm=W_nm, N=train_size)
        inv_eig_val_K = np.diag(1 / eig_val_K / N2)
    ...
    if args.hparam == 'lmo':
        ...
        def LMO_err(params_l, M=train_size*0.1):
            EYEN = np.eye(train_size)
            N2 = train_size ** 2
            ...
            if nystr:
                ...
                print('here')
                # L_nystr = bl * bl * eig_vec_L @ np.diag(eig_val_L) @ eig_vec_L.T + JITTER * EYEN
                print('shapes: ', eig_vec_L.shape, eig_val_L.shape)
                print('1', eig_vec_L @ np.diag(eig_val_L) @ eig_vec_L.T)
                K_L = eig_vec_L @ np.diag(eig_val_L) @ eig_vec_L.T
                L = bl * bl * K_L + JITTER * EYEN
                tmp_mat_ = L @ eig_vec_K
                print('here')
                tmp_mat = (bl * bl * eig_vec_L @ np.diag(eig_val_L) @ eig_vec_L.T + JITTER * EYEN) @ eig_vec_K
                print('here')
                C = (bl * bl * eig_vec_L @ np.diag(eig_val_L) @ eig_vec_L.T + JITTER * EYEN) - tmp_mat @ anp.linalg.inv(
                    eig_vec_K.T @ tmp_mat / N2 + inv_eig_val_K) @ tmp_mat.T / N2
                print('here')
                c = C @ (eig_vec_K @ np.diag(eig_val_K) @ eig_vec_K.T / N2) * N2
                print('herehere')
                del tmp_mat
        ...
        assert opt_params_l is not None
    ...
    alpha = compute_alpha(train_size=train_size, eig_vec_K=eig_vec_K, X=X, Y=Y, W=W,
    ...
    losses = compute_losses(params_l=params_l_final, ax=ax, w_samples=w_samples, y_samples=y_samples, y_axz=y_axz,
                                            ...
                                            W=W, eig_vec_K=eig_vec_K, eig_val_K=eig_val_K, inv_eig_val_K=inv_eig_val_K,
...
def do_bl_hparam_analysis_plots(PATH, args, train_size, bl_min, bl_max, bl_mesh_size, data_seed, **h_param_results_dict):
    ...
    for var_str in h_param_results_dict.keys():
        ...
        print('save path: ', os.path.join(PATH, str(date.today()), args.sem+'_seed'+str(data_seed), 'hparam_anal_{}_trainsz{}_offset{}.png'.format(var_str, train_size, args.offset)))
        plt.savefig(os.path.join(PATH, str(date.today()), args.sem+'_seed'+str(data_seed), 'hparam_anal_{}_trainsz{}_offset{}.png'.format(var_str, train_size, args.offset)))
        plt.close()
        ...
        plt.xlabel('Hyperparameter labels'), plt.ylabel('causal_MAE/{}-rescaled'.format(var_str)), plt.legend()
        print('save path: ', os.path.join(PATH, str(date.today()), args.sem+'_seed'+str(data_seed), 'hparam_anal_{}_trainsz{}_offset{}_inversc.png'.format(var_str, train_size, args.offset)))
        plt.savefig(os.path.join(PATH, str(date.today()), args.sem+'_seed'+str(data_seed), 'hparam_anal_{}_trainsz{}_offset{}_inversc.png'.format(var_str, train_size, args.offset)))
        plt.close()
...
def cube_search(al_diff_min, al_diff_max, al_mesh_size, bl_min, bl_max, bl_mesh_size, args, PATH, train_size, seed, data_seed):
    ...
    # do_bl_hparam_analysis_plots(PATH=PATH, args=args, train_size=train_size,
    ...
    print('best mae for {} found at params_l: {} using hparam search method: {}'.format(sname, best_hparams_l,
                                                                                      args.hparam))
    with open(os.path.join(PATH, str(date.today()), args.sem+'_seed'+str(data_seed), 'best_params_l_cube_trainsz{}_offset{}.txt'.format(train_size, args.offset)), 'w') as f:
        f.write('best al: {}, best bl: {:.3f}'.format(best_hparams_l[0] + al_default, best_hparams_l[1]))

    return best_hparams_l, best_ate_est


def hyparameter_selection(al_diff_min, al_diff_max, al_mesh_size, bl_min, bl_max, bl_mesh_size, args, PATH, train_size, seed, data_seed):
    if args.hparam == 'cube':
        best_hparams_l, best_ate_est = cube_search(al_diff_min=al_diff_min, al_diff_max=al_diff_max, al_mesh_size=al_mesh_size,
                                                   bl_min=bl_min, bl_max=bl_max, bl_mesh_size=bl_mesh_size,
                                                   args=args, PATH=PATH, train_size=train_size, data_seed=data_seed, seed=seed)
        return best_hparams_l, best_ate_est
...
def run_pmmr_rkhs(seed, al_diff_search_range, bl_search_range, train_sizes, data_seeds, args):
    ...
    for train_size in train_sizes:
        ...
        for data_seed in data_seeds:
            ...
            EY_do_A_gt = np.load(ROOT_PATH + "/data/zoo/" + sname + '/do_A_{}_seed{}.npz'.format(args.sem, data_seed))[
                'gt_EY_do_A']
            ...
            best_hparams_l, best_ate_est = hyparameter_selection(seed=seed, al_diff_max=al_diff_max, al_diff_min=al_diff_min,
                                                                 ...
                                                                 bl_mesh_size=bl_mesh_size, args=args, PATH=PATH,
            ...
            best_causal_mae, best_causal_std, best_causal_rel_err = evaluate_ate_est(ate_est=best_ate_est,
                                                                                     ate_gt=EY_do_A_gt)
            np.savez(os.path.join(PATH, str(date.today()), args.sem + '_seed' + str(data_seed),
