...
bn_max, bn_min, b_mesh_size = (40, 3, 3)
...
def experiment(sname, seed, param_arg, train_size, nystr=False, args=None, PATH=None, hparam=None):
    ...
    def get_causal_effect(params, do_A, w, Y_scaler):
        ...
        ate_L_bin = indicator_kern(aw_rep[:, aw_bin_idx:], X[:, aw_bin_idx:])
    ...
    Z_scaled, test_Z_scaled = np.concatenate([Z_cts_scaled, train.z[:train_size, z_bin_idx:]], axis=-1), np.concatenate([test_Z_cts_scaled, test.z[:test_size, z_bin_idx:]], axis=-1)
    ...
    W = np.prod(np.array(W_list), axis=0) * W_cts

    print('sum W: ', np.sum(W))

    L0_cts, test_L0_cts = _sqdist(X[:, :aw_bin_idx], None), _sqdist(test_X[:, :aw_bin_idx], X[:, :aw_bin_idx])
    L_bin, test_L_bin = indicator_kern(X[:, aw_bin_idx:], X[:, aw_bin_idx:]), indicator_kern(test_X[:, aw_bin_idx:], X[:, aw_bin_idx:])
    t4 = time.time()
    print('t4 - t3', t4-t3)
    # measure time
...
if __name__ == '__main__':
    snames = ["sim_1d_no_x"]
    for train_size in [7000, 3000, 1000]:
        b_max, b_min = bn_max / train_size, bn_min / train_size
        for data_seed in [100, 200, 300, 400, 500]:
            for sname in snames:
                PATH = ROOT_PATH + "/MMR_proxy/results/zoo/" + sname + "/"
                if not os.path.exists(os.path.join(PATH, str(date.today()), args.sem+'_seed'+str(data_seed))):
                    os.makedirs(os.path.join(PATH, str(date.today()), args.sem+'_seed'+str(data_seed)), exist_ok=True)
