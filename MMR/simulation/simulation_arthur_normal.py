...
def main(args, seed):
    ...
    Z = U2 + np.random.normal(0, 1, size=(N[-1], 1))
    ...
    W = U1 + np.random.normal(0, 1, size=(N[-1], 1)) * wStd
    ...
    A = U2 + np.random.normal(0, 1, size=U2.shape) * aStd
    ...
    Y = U2 * np.cos(2 * (A + 0.3 * U1 + 0.2)) + np.random.normal(0, 1, size=A.shape)
    ...
    # plotting
    D = pd.DataFrame([U[:10000, 0], U[:10000, 1], train_a[:10000].flatten(), train_y[:10000].flatten(), train_z[:10000].flatten(),
                      train_w[:10000].flatten()]).T
