...
seed = 0
file_path = os.path.join(os.path.dirname(__file__), '_deaner_edu_split_into_azwyx.npz')
...
def plot_real_data(file_path, root_path):
    ...
    a, w, z, y, x = data['a'], data['w'], data['z'], data['y'], data['x']
    D = pd.DataFrame([a[:1000,0], w[:1000,0], w[:1000,1], w[:1000,2], w[:1000,3], z[:1000,0], z[:1000, 1], z[:1000,2], y[:1000, 0], y[:1000, 1]]).T
    D.columns = ['A', 'W1', 'W2', 'W3', 'W4', 'Z1', 'Z2', 'Z3', 'Y1', 'Y2']
    sns.set_theme(font="tahoma", font_scale=1)
    print('here')
    sns.pairplot(D), plt.savefig(os.path.join(root_path, 'simulation', 'edu', '_orig_pairwise.png')), plt.close()
    calculate_correlation_coeff_edu(x=x, z=z, w=w, a=a, y=y, root_path=root_path, save_name='_corr_orig')
...
def load_split_and_standardise(file_path, root_path):
    ...
    D = pd.DataFrame([tr_data_scaled['u'][:1000,0], tr_data_scaled['a'][:1000,0], tr_data_scaled['w'][:1000,0], tr_data_scaled['w'][:1000,1], tr_data_scaled['z'][:1000,0], tr_data_scaled['y'][:1000]]).T
    D.columns = ['U', 'A', 'W1', 'W2', 'Z', 'Y']
    sns.set_theme(font="tahoma", font_scale=1)
    sns.pairplot(D), plt.savefig(os.path.join(root_path, 'simulation', 'edu', 'scaled_orig_pairwise.png')), plt.close()
    calculate_correlation_coeff_edu(x=tr_data_scaled['x'],
                                    u=tr_data_scaled['u'],
                                    z=tr_data_scaled['z'],
                                    w=tr_data_scaled['w'],
                                    a=tr_data_scaled['a'],
...
def generate_data_sample(x_samples_transformed, u_model, z_model, w_model, y_model, a_model, root_path, reps):
    ...
    if not os.path.exists(os.path.join(root_path, 'simulation', 'edu')):
        os.mkdir(os.path.join(root_path, 'simulation', 'edu'))
    if not os.path.exists(os.path.join(root_path, 'data/zoo/edu_fitted_no_x/')):
        os.mkdir(os.path.join(root_path, 'data/zoo/edu_fitted_no_x/'))
    if not os.path.exists(os.path.join(root_path, 'data/zoo/edu_fitted_with_x/')):
        os.mkdir(os.path.join(root_path, 'data/zoo/edu_fitted_with_x/'))
    ...
    sns.set_theme(font="tahoma", font_scale=1)
    sns.pairplot(D), plt.savefig(os.path.join(root_path, 'simulation', 'edu', 'fitted_pairwise.png')), plt.close()
    calculate_correlation_coeff_edu(x=x_samples_transformed[:1000],
                                u=u_samples[:1000],
                                z=z_samples[:1000],
                                w=w_samples[:1000],
                                a=a_samples[:1000],
    ...
    np.savez(os.path.join(root_path, 'data/zoo/edu_fitted_no_x/edu.npz'), u=u_samples, z=z_samples, w=w_samples, a=a_samples, y=y_samples)
    np.savez(os.path.join(root_path, 'data/zoo/edu_fitted_with_x/edu.npz'), u=u_samples, x=x_samples_transformed, z=z_samples, w=w_samples, a=a_samples, y=y_samples)
    ...
    np.savez(os.path.join(root_path, 'data/zoo/edu_fitted_no_x/main_.npz'), **savez_inp_no_x)
    np.savez(os.path.join(root_path, 'data/zoo/edu_fitted_with_x/main_.npz'), **savez_inp_with_x)
    ...
    np.savez(os.path.join(root_path, 'data/zoo/edu_fitted_with_x/do_A_.npz'),
             do_A = do_A,
             gt_EY_do_A = EY_do_A)
